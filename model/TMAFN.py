...
'''
三。 Multi-head-cross Attention (MHCA)
'''
class MHCA(nn.Module):
    def __init__(self, in_channels, Nh=4, kernel_size=3, stride=1):
        ...
        self.qkv_conv1 = nn.Conv2d(self.in_channels, self.dq+ self.dk+ self.dv, kernel_size = self.kernel_size,
                                   stride = stride, padding = self.padding)
        self.qkv_conv2 = nn.Conv2d(self.in_channels, self.dq+ self.dk+ self.dv, kernel_size = self.kernel_size,
                                   stride = stride, padding = self.padding)
        self.qkv_conv12 = nn.Conv2d(self.in_channels, self.dq+ self.dk+ self.dv, kernel_size = self.kernel_size,
                                    stride = stride, padding = self.padding)
    ...
    def forward(self, img1, img2, abu):
        ...
        combined_q = torch.cat((flat_q1, flat_q12, flat_q2), dim=2) # (batch, Nh, dkh*3, height* width)
        combined_k = torch.cat((flat_k1, flat_k12, flat_k2), dim=2)
        combined_v = torch.cat((flat_v1, flat_v12, flat_v2), dim=2)

        logits = torch.matmul(combined_q.transpose(2, 3), combined_k)   # (batch, Nh, height * width, height * width)
        weights1 = nn.Softmax(dim=-1)(logits)  # (batch, Nh, height * width, height * width)

        # attn_out
        # (batch, Nh, height * width, dvh)
        MHCA_out = torch.matmul(weights1, combined_v.transpose(2, 3))   # (batch, Nh, height * width，dkh*3)

        dvh = self.dv // self.Nh
        img1_out, abu_out, img2_out = torch.split(MHCA_out, [dvh, dvh, dvh], dim = 3)  # (batch, Nh, height * width，dvh)

        Nh_shape = (batch, self.Nh, dvh, height, width)
        img1_out = torch.reshape(img1_out, Nh_shape)   # (batch, Nh, dvh, height, width)
        img2_out = torch.reshape(img2_out, Nh_shape)
        abu_out = torch.reshape(abu_out, Nh_shape)
        # combine_heads_2d
        ...
        # Residual connection
        img1_out = self.attn_out1(img1_out)
        img2_out = self.attn_out1(img2_out)
...
class TMAFN(nn.Module):
    def __init__(self,img_ch, abu_ch): 
        ...
        # 1. MHCA
        self.MHCA1 = MHCA(in_channels = low_ch, Nh=4)
        self.MHCA2 = MHCA(in_channels = mid_ch, Nh=4)
        self.MHCA3 = MHCA(in_channels = deep_ch, Nh=4)
        ...
        self.abu_convd2m = nn.Sequential(
            nn.ConvTranspose2d(deep_ch, mid_ch, kernel_size=3),
            nn.BatchNorm2d(mid_ch),
            nn.ReLU(inplace=True),
        )
        self.sumconv = nn.Sequential(
            nn.Conv2d(mid_ch*3, mid_ch, kernel_size=1),
            nn.BatchNorm2d(mid_ch),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, img1, img2, abu): # input data size: b, ch,h,w
        ...
        img1_feature1, img2_feature1, abu_feature1 = self.MHCA1(out1, out2, out12)
        ...
        img1_feature2, img2_feature2, abu_feature2 = self.MHCA2(out1, out2, out12)
        ...
        img1_feature3, img2_feature3, abu_feature3 = self.MHCA3(out1, out2, out12)

        # abu msrf
        # 采样
        abu_map_up = self.abu_convl2m(abu_feature1)    # b,mid_ch, mid_scale, mid_scale
        abu_map_mid = self.abu_convm2m(abu_feature2)    # b,mid_ch, mid_scale, mid_scale
        abu_map_down = self.abu_convd2m(abu_feature3)    # b,mid_ch, mid_scale, mid_scale

        abu_map_sum = torch.cat((abu_map_up, abu_map_mid, abu_map_down), dim=1) # b,mid_ch*3, mid_scale, mid_scale

        net_out = self.sumconv(abu_map_sum) + abu_map_up + abu_map_mid + abu_map_down
