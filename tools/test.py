...
def load_model(model, pretrained_path, device):
    ...
    if device == torch.device('cpu'):
        ...
        pretrained_dict = torch.load(pretrained_path, map_location=lambda storage, loc: storage.cuda(cuda_index))['model']
...
def test(test_data, origin_gt, model, device, cfg):
    ...
    for batch_idx, batch_sample in enumerate(batch_data):
        ...
        img1, img2, abu, target, indices = batch_sample
        img1 = img1.to(device)
        img2 = img2.to(device)
        abu = abu.to(device)
        with torch.no_grad():
        ...
        if target.sum() > 0:
            ...
            raise ValueError('Wrong gt value')
